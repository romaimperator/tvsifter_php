...
echo '          <th width="25">' . $html->link($this->Html->image('/acls/img/add.png', array('alt' => 'Add Permission')), array('action' => 'add', $aco_id), array('escape' => false, 'title' => 'Add Permission')) . '</th>';
...
if (!empty($permissions)) {
    ...
    foreach($permissions as $i) {
        ...
        echo '  <td>' . $form->checkbox('Permission.delete.' . $i['Permission']['id']) . '</td>';
        echo '  <td>' . $html->link($this->Html->image('/acls/img/edit.png', array('alt' => 'Edit Permission')), array('action' => 'edit', $aco_id, $i['Permission']['id']), array('escape' => false, 'title' => 'Edit Permission')) . '</td>';
